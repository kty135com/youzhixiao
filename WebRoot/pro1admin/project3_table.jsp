			...
			<table>
				...
				<s:iterator value="p2s" var="p3" status="p3t">
					<tr>
						...
						<td><s:if test="#p3.state==0">建设中</s:if> <s:if
								test="#p3.state==1">待通过</s:if> <s:if test="#p3.state==2">已通过</s:if></td>
						<td class="table-operate ue-clear"><s:if test="qqq==2">
								<a
									href="<%=basePath%>project2/project2_pend3Bypro1admin?id=<s:property value="#p3.id"/>"
									class="add">查看</a>
							</s:if> <s:else>
								<a
									href="<%=basePath%>project2/project2_pend3?id=<s:property value="#p3.id"/>"
									class="add">查看</a>
							</s:else>
					</tr>
				</s:iterator>
